...
// public-schedule Trigger.dev task) and renders one week at a time (Sun-Sat,
// seven columns) for a single chosen floor (VUCA / ART / LIVO). Week navigation
...
(function () {
  ...
  function fmtRange(sun, sat) {
    var sM = MONTHS[sun.getUTCMonth()], eM = MONTHS[sat.getUTCMonth()];
    if (sM === eM) return sun.getUTCDate() + '-' + sat.getUTCDate() + ' ' + eM;
    return sun.getUTCDate() + ' ' + sM + ' - ' + sat.getUTCDate() + ' ' + eM;
  }
  ...
  function render() {
    ...
    if (weekLabel) weekLabel.textContent = fmtRange(sun, sat);
    ...
    // Sun (0) through Sat (6), seven columns.
    for (var i = 0; i < 7; i++) {
